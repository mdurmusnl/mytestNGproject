...
//            We have different options. I choose if URL have auth keyword,
//                  it means we are on the login page   we can also assert by using log in button is displayed or not
      Assert.assertTrue(Driver.getDriver().getCurrentUrl().contains("auth"));
         // Assert.assertTrue(orangeHRMLoginPage.submitButton.isDisplayed());  also possible  i sued orangehrmliginpage bcs we reach home page after log out
